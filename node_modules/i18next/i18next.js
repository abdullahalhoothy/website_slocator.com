              ...
              if (needsContextHandling) {
                ...
                if (needsPluralHandling) {
                  if (opt.ordinal && pluralSuffix.startsWith(ordinalPrefix)) {
                    finalKeys.push(
                      contextKey +
                        pluralSuffix.replace(ordinalPrefix, this.options.pluralSeparator),
                    )
                  }
  ...
  class I18n extends EventEmitter {
    ...
    loadLanguages(lngs, callback) {
      ...
      const newLngs = lngs.filter(
        lng => !preloaded.includes(lng) && this.services.languageUtils.isSupportedCode(lng),
      )
      if (!newLngs.length) {
        if (callback) callback()
        return Promise.resolve()
    ...
    dir(lng) {
      if (!lng)
        lng =
          this.resolvedLanguage || (this.languages?.length > 0 ? this.languages[0] : this.language)
      if (!lng) return 'rtl'
      try {
      ...
      return rtlLngs.includes(languageUtils.getLanguagePartFromCode(lng)) ||
