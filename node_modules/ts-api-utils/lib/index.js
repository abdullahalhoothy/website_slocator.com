...
function* iterateComments(node, sourceFile = node.getSourceFile()) {
  ...
  for (const token of iterateTokens(node, sourceFile)) {
    ...
    if (notJsx || canHaveTrailingTrivia(token)) {
      yield* collectComments(callback => {
        ts9.forEachTrailingCommentRange(fullText, token.end, callback)
      }, fullText)
    }
  }
}
function canHaveTrailingTrivia(token) {
  switch (token.kind) {
    case ts9.SyntaxKind.CloseBraceToken:
      return (
        token.parent.kind !== ts9.SyntaxKind.JsxExpression ||
...
function collectComments(execute, fullText) {
  ...
  execute((pos, end, kind) => {
    ...
    const value = text.slice(2, kind === ts9.SyntaxKind.SingleLineCommentTrivia ? void 0 : -2)
    ...
    case 'allowSyntheticDefaultImports':
      return options.allowSyntheticDefaultImports !== void 0
        ...
        : isCompilerOptionEnabled(options, 'esModuleInterop') ||
            options.module === ts9.ModuleKind.System
    case 'alwaysStrict':
    case 'noImplicitAny':
    case 'noImplicitThis':
...
function getAccessKind(node) {
  ...
  switch (parent.kind) {
    ...
    case ts9.SyntaxKind.BinaryExpression:
      return parent.right === node
        ...
        : !isAssignmentKind(parent.operatorToken.kind)
          ? 1 /* Read */
          : parent.operatorToken.kind === ts9.SyntaxKind.EqualsToken
            ? 2 /* Write */
            : 3 /* ReadWrite */
    ...
    case ts9.SyntaxKind.ExpressionWithTypeArguments:
      return parent.parent.token === ts9.SyntaxKind.ExtendsKeyword &&
        parent.parent.parent.kind !== ts9.SyntaxKind.InterfaceDeclaration
        ? 1 /* Read */
        : 0 /* None */
    ...
    case ts9.SyntaxKind.PrefixUnaryExpression:
      return parent.operator === ts9.SyntaxKind.PlusPlusToken ||
        parent.operator === ts9.SyntaxKind.MinusMinusToken
        ? 3 /* ReadWrite */
        : 1 /* Read */
...
function isInDestructuringAssignment(node) {
  ...
  while (true) {
    switch (node.parent.kind) {
      ...
      case ts9.SyntaxKind.BinaryExpression:
        return (
          node.parent.left === node && node.parent.operatorToken.kind === ts9.SyntaxKind.EqualsToken
        )
      case ts9.SyntaxKind.ForOfStatement:
...
function hasDecorators(node) {
  return (
    ts9.isParameter(node) ||
    ts9.isPropertyDeclaration(node) ||
    ts9.isMethodDeclaration(node) ||
    ts9.isGetAccessorDeclaration(node) ||
    ts9.isSetAccessorDeclaration(node) ||
    ts9.isClassExpression(node) ||
    ts9.isClassDeclaration(node)
  )
}
function hasExpressionInitializer(node) {
  return (
    ts9.isVariableDeclaration(node) ||
    ts9.isParameter(node) ||
    ts9.isBindingElement(node) ||
    ts9.isPropertyDeclaration(node) ||
    ts9.isPropertyAssignment(node) ||
    ts9.isEnumMember(node)
  )
}
function hasInitializer(node) {
  return (
    hasExpressionInitializer(node) ||
    ts9.isForStatement(node) ||
    ts9.isForInStatement(node) ||
    ts9.isForOfStatement(node) ||
    ts9.isJsxAttribute(node)
  )
}
function hasJSDoc(node) {
  if (
    ...
    ts9.isArrowFunction(node) ||
    ts9.isBlock(node) ||
    ts9.isBreakStatement(node) ||
    ts9.isCallSignatureDeclaration(node) ||
    ts9.isCaseClause(node) || // eslint-disable-next-line @typescript-eslint/no-deprecated -- Keep compatibility with ts <5
    isClassLikeDeclaration(node) ||
    ts9.isConstructorDeclaration(node) ||
    ts9.isConstructorTypeNode(node) ||
    ts9.isConstructSignatureDeclaration(node) ||
    ts9.isContinueStatement(node) ||
    ts9.isDebuggerStatement(node) ||
    ts9.isDoStatement(node) ||
    ts9.isEmptyStatement(node) ||
    isEndOfFileToken(node) ||
    ts9.isEnumDeclaration(node) ||
    ts9.isEnumMember(node) ||
    ts9.isExportAssignment(node) ||
    ts9.isExportDeclaration(node) ||
    ts9.isExportSpecifier(node) ||
    ts9.isExpressionStatement(node) ||
    ts9.isForInStatement(node) ||
    ts9.isForOfStatement(node) ||
    ts9.isForStatement(node) ||
    ts9.isFunctionDeclaration(node) ||
    ts9.isFunctionExpression(node) ||
    ts9.isFunctionTypeNode(node) ||
    ts9.isIfStatement(node) ||
    ts9.isImportDeclaration(node) ||
    ts9.isImportEqualsDeclaration(node) ||
    ts9.isIndexSignatureDeclaration(node) ||
    ts9.isInterfaceDeclaration(node) ||
    ts9.isJSDocFunctionType(node) ||
    ts9.isLabeledStatement(node) ||
    ts9.isMethodDeclaration(node) ||
    ts9.isMethodSignature(node) ||
    ts9.isModuleDeclaration(node) ||
    ts9.isNamedTupleMember(node) ||
    ts9.isNamespaceExportDeclaration(node) ||
    ts9.isParameter(node) ||
    ts9.isParenthesizedExpression(node) ||
    ts9.isPropertyAssignment(node) ||
    ts9.isPropertyDeclaration(node) ||
    ts9.isPropertySignature(node) ||
    ts9.isReturnStatement(node) ||
    ts9.isShorthandPropertyAssignment(node) ||
    ts9.isSpreadAssignment(node) ||
    ts9.isSwitchStatement(node) ||
    ts9.isThrowStatement(node) ||
    ts9.isTryStatement(node) ||
    ts9.isTypeAliasDeclaration(node) ||
    ts9.isVariableDeclaration(node) ||
    ts9.isVariableStatement(node) ||
    ts9.isWhileStatement(node) ||
    ts9.isWithStatement(node)
  ) {
    return true
  }
  ...
  if (
    ...
    (ts9.isBinaryExpression(node) ||
      ts9.isElementAccessExpression(node) ||
      ts9.isIdentifier(node) ||
      ts9.isJSDocSignature(node) ||
      ts9.isObjectLiteralExpression(node) ||
      ts9.isPropertyAccessExpression(node) ||
      ts9.isTypeParameterDeclaration(node))
  ) {
    return true
  }
  return false
...
function hasType(node) {
  return (
    ...
    ts9.isPropertySignature(node) ||
    ts9.isPropertyDeclaration(node) ||
    ts9.isTypePredicateNode(node) ||
    ts9.isParenthesizedTypeNode(node) ||
    ts9.isTypeOperatorNode(node) ||
    ts9.isMappedTypeNode(node) ||
    ts9.isAssertionExpression(node) ||
    ts9.isTypeAliasDeclaration(node) ||
    ts9.isJSDocTypeExpression(node) ||
    ts9.isJSDocNonNullableType(node) ||
    ts9.isJSDocNullableType(node) ||
    ts9.isJSDocOptionalType(node) ||
    ts9.isJSDocVariadicType(node)
  )
}
function hasTypeArguments(node) {
  return (
    ts9.isCallExpression(node) ||
    ts9.isNewExpression(node) ||
    ts9.isTaggedTemplateExpression(node) ||
    ts9.isJsxOpeningElement(node) ||
    ts9.isJsxSelfClosingElement(node)
  )
}
...
function isBlockLike(node) {
  return (
    ts9.isSourceFile(node) ||
    ts9.isBlock(node) ||
    ts9.isModuleBlock(node) ||
    ts9.isCaseOrDefaultClause(node)
  )
}
...
function isDeclarationName(node) {
  return (
    ts9.isIdentifier(node) ||
    ts9.isPrivateIdentifier(node) ||
    ts9.isStringLiteralLike(node) ||
    ts9.isNumericLiteral(node) ||
    ts9.isComputedPropertyName(node) ||
    ts9.isElementAccessExpression(node) ||
...
function isDeclarationWithTypeParameterChildren(node) {
  return (
    ...
    ts9.isInterfaceDeclaration(node) ||
    ts9.isTypeAliasDeclaration(node) ||
    ts9.isJSDocTemplateTag(node)
  )
}
function isDeclarationWithTypeParameters(node) {
  return (
    isDeclarationWithTypeParameterChildren(node) ||
    ts9.isJSDocTypedefTag(node) ||
    ts9.isJSDocCallbackTag(node) ||
    ts9.isJSDocSignature(node)
  )
}
function isDestructuringPattern(node) {
  return (
    isBindingPattern(node) ||
    ts9.isObjectLiteralExpression(node) ||
    ts9.isArrayLiteralExpression(node)
  )
}
...
function isJsxAttributeValue(node) {
  return (
    ts9.isStringLiteral(node) ||
    ts9.isJsxExpression(node) ||
    ts9.isJsxElement(node) ||
    ts9.isJsxSelfClosingElement(node) ||
    ts9.isJsxFragment(node)
  )
}
...
function isLiteralToken(node) {
  return (
    ts9.isNumericLiteral(node) ||
    ts9.isBigIntLiteral(node) ||
    ts9.isStringLiteral(node) ||
    ts9.isJsxText(node) ||
    ts9.isRegularExpressionLiteral(node) ||
    ts9.isNoSubstitutionTemplateLiteral(node)
  )
}
...
function isSignatureDeclaration(node) {
  return (
    ...
    ts9.isFunctionDeclaration(node) ||
    ts9.isMethodDeclaration(node) ||
    ts9.isConstructorDeclaration(node) || // eslint-disable-next-line @typescript-eslint/no-deprecated -- Keep compatibility with ts <5
    isAccessorDeclaration(node) ||
    ts9.isFunctionExpression(node) ||
    ts9.isArrowFunction(node)
  )
...
function isTypeOnlyCompatibleAliasDeclaration(node) {
  if (
    ts9.isImportClause(node) ||
    ts9.isImportEqualsDeclaration(node) ||
    ts9.isNamespaceImport(node) ||
    ts9.isImportOrExportSpecifier(node)
  ) {
    return true
  }
...
function isVariableLikeDeclaration(node) {
  return (
    ...
    ts9.isShorthandPropertyAssignment(node) ||
    ts9.isEnumMember(node) ||
    ts9.isJSDocPropertyTag(node) ||
    ts9.isJSDocParameterTag(node)
  )
}
...
function isConstAssertionExpression(node) {
  return (
    ts9.isTypeReferenceNode(node.type) &&
    ts9.isIdentifier(node.type.typeName) &&
...
function isJSDocNamespaceDeclaration(node) {
  return (
    ts9.isModuleDeclaration(node) &&
    ts9.isIdentifier(node.name) &&
    (node.body === void 0 || isJSDocNamespaceBody(node.body))
  )
}
function isJsxTagNamePropertyAccess(node) {
  return (
    ts9.isPropertyAccessExpression(node) && // eslint-disable-next-line @typescript-eslint/no-deprecated -- Keep compatibility with ts < 5
...
var IntrinsicTypeFlags =
  ts9.TypeFlags.Intrinsic ??
  ts9.TypeFlags.Any |
    ts9.TypeFlags.Unknown |
    ts9.TypeFlags.String |
    ts9.TypeFlags.Number |
    ts9.TypeFlags.BigInt |
    ts9.TypeFlags.Boolean |
    ts9.TypeFlags.BooleanLiteral |
    ts9.TypeFlags.ESSymbol |
    ts9.TypeFlags.Void |
    ts9.TypeFlags.Undefined |
    ts9.TypeFlags.Null |
    ts9.TypeFlags.Never |
...
function getWellKnownSymbolPropertyOfType(type, wellKnownSymbolName, typeChecker) {
  ...
  for (const prop of type.getProperties()) {
    ...
    if (
      ...
      !ts9.isComputedPropertyName(declaration.name)
    ) {
      continue
    }
...
function isBindableObjectDefinePropertyCall(node) {
  return (
    ...
    ts9.isPropertyAccessExpression(node.expression) &&
    node.expression.name.escapedText === 'defineProperty' &&
    ts9.isIdentifier(node.expression.expression) &&
    ...
    outer: switch (parent.kind) {
      ...
      case ts9.SyntaxKind.CallExpression: {
        ...
        return (
          ...
          isTransientSymbolLinksFlagSet(propertySymbol.links, ts9.CheckFlags.Readonly)
        )
      }
      case ts9.SyntaxKind.PrefixUnaryExpression:
...
function isFalsyType(type) {
  if (isTypeFlagSet(type, ts9.TypeFlags.Undefined | ts9.TypeFlags.Null | ts9.TypeFlags.Void)) {
    return true
  }
    ...
    if (getPropertyOfType(subType, name) === void 0) {
      const index =
        (isNumericPropertyName(name)
          ? typeChecker.getIndexInfoOfType(subType, ts9.IndexKind.Number)
          : void 0) ?? typeChecker.getIndexInfoOfType(subType, ts9.IndexKind.String)
      if (index?.isReadonly) {
        if (seenProperty) {
          return true
        }
...
function symbolHasReadonlyDeclaration(symbol, typeChecker) {
  return !!(
    (symbol.flags & ts9.SymbolFlags.Accessor) === ts9.SymbolFlags.GetAccessor ||
    symbol.declarations?.some(
      node =>
        isModifierFlagSet(node, ts9.ModifierFlags.Readonly) ||
        (ts9.isVariableDeclaration(node) && isNodeFlagSet(node.parent, ts9.NodeFlags.Const)) ||
        (ts9.isCallExpression(node) && isReadonlyAssignmentDeclaration(node, typeChecker)) ||
        ts9.isEnumMember(node) ||
        ((ts9.isPropertyAssignment(node) || ts9.isShorthandPropertyAssignment(node)) &&
...
function isReadonlyAssignmentDeclaration(node, typeChecker) {
  ...
  const writableType =
    ...
    ts9.isPropertyAssignment(writableProp.valueDeclaration)
...
function identifierToKeywordKind(node) {
  return 'originalKeywordKind' in node
    ...
    : ts9.identifierToKeywordKind(node)
}
...
function getDeclarationDomain(node) {
  switch (node.parent.kind) {
    ...
    case ts9.SyntaxKind.Parameter:
      if (
        node.parent.parent.kind === ts9.SyntaxKind.IndexSignature ||
        identifierToKeywordKind(node) === ts9.SyntaxKind.ThisKeyword
      ) {
        return
      }
    // falls through
  ...
  if (propertyName.kind === ts9.SyntaxKind.ComputedPropertyName) {
    ...
    if (ts9.isPrefixUnaryExpression(expression)) {
      ...
      switch (expression.operator) {
        ...
        // falls through
        case ts9.SyntaxKind.PlusToken:
          return ts9.isNumericLiteral(expression.operand)
            ? `${negate ? '-' : ''}${expression.operand.text}`
            : ts9.isBigIntLiteral(expression.operand)
...
function getUsageDomain(node) {
  ...
  switch (parent.kind) {
    ...
    case ts9.SyntaxKind.ExportSpecifier:
      ...
      break
    case ts9.SyntaxKind.ExpressionWithTypeArguments:
      return parent.parent.token === ts9.SyntaxKind.ImplementsKeyword ||
        parent.parent.parent.kind === ts9.SyntaxKind.InterfaceDeclaration
        ? 2 /* Type */
        : 4 /* Value */
...
var UsageWalker = class {
  ...
  getUsage(sourceFile) {
    ...
    const cb = node => {
      ...
      switch (node.kind) {
        ...
        case ts9.SyntaxKind.EnumDeclaration:
          ...
          return continueWithScope(
            ...
            this.#scope.createOrReuseEnumScope(
              node.name.text,
              includesModifier(node.modifiers, ts9.SyntaxKind.ExportKeyword),
            ),
          )
        case ts9.SyntaxKind.EnumMember:
        ...
        case ts9.SyntaxKind.Parameter:
          if (
            node.parent.kind !== ts9.SyntaxKind.IndexSignature &&
            (node.name.kind !== ts9.SyntaxKind.Identifier ||
              identifierToKeywordKind(node.name) !== ts9.SyntaxKind.ThisKeyword)
          ) {
  ...
  #handleFunctionLikeDeclaration(node, cb, varCb) {
    ...
    const scope = (this.#scope =
      node.kind === ts9.SyntaxKind.FunctionExpression && node.name !== void 0
  ...
  #handleVariableDeclaration(declarationList) {
    ...
    const exported =
      declarationList.parent.kind === ts9.SyntaxKind.VariableStatement &&
      includesModifier(declarationList.parent.modifiers, ts9.SyntaxKind.ExportKeyword)
...
function containsExportStatement(block) {
  for (const statement of block.statements) {
    if (
      statement.kind === ts9.SyntaxKind.ExportDeclaration ||
      statement.kind === ts9.SyntaxKind.ExportAssignment
    ) {
      return true
    }
  }
...
function isNamespaceExported(node) {
  return (
    node.parent.kind === ts9.SyntaxKind.ModuleDeclaration ||
    includesModifier(node.modifiers, ts9.SyntaxKind.ExportKeyword)
  )
}
