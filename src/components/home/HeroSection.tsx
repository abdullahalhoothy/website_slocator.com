        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          <div className="w-full lg:w-5/12 relative flex justify-center order-2 lg:order-1">
            <div className="relative w-full max-w-[480px]">
              <img src="/assets/images/saudie man.png" className="w-full h-auto object-contain relative z-10" />
              
              <div className={`absolute top-[20%] ${isRTL ? '-right-8' : '-left-8'} z-20 bg-white rounded-xl shadow-2xl p-4 border border-slate-100 flex items-center gap-3 animate-bounce-slow`}>
                <div className="bg-purple-50 p-2 rounded-lg"><Map className="w-5 h-5 text-purple-600" /></div>
                <span className="font-bold text-slate-800 text-sm">{t('hero.routePlanner')}</span>
              </div>

              <div className={`absolute bottom-[15%] ${isRTL ? '-left-8' : '-right-8'} z-20 bg-white rounded-xl shadow-2xl p-4 border border-slate-100 flex items-center gap-3 animate-bounce-slow-delayed`}>
                <div className="bg-[#44E559]/10 p-2 rounded-lg"><Layers className="w-5 h-5 text-[#3bc24d]" /></div>
                <span className="font-bold text-slate-800 text-sm">{t('hero.territoryOptimization')}</span>
              </div>
            </div>
          </div>
