...
export const CompanyInfo: React.FC = () => {
  ...
  return (
    <section
      className={`bg-white pt-24 pb-16 overflow-hidden ${isRTL ? 'text-right' : 'text-left'}`}
    >
        ...
        <div className="text-center mb-24">
          <FadeIn direction="up">
            ...
            <p className="text-lg text-slate-500 font-light max-w-2xl mx-auto mb-12">
              {t('about.subtitle', 'Our journey in numbers')}
            </p>
          </FadeIn>
          ...
          <div className="flex flex-wrap justify-center gap-6">
            <FadeIn delay={100} direction="up">
              <div className="bg-white border border-slate-100 shadow-sm rounded-lg p-8 min-w-[250px] flex flex-col items-center">
                ...
                <div className="text-slate-500 font-light text-sm uppercase tracking-wider">
        ...
        <div className="text-center mb-12">
          <FadeIn direction="up">
            ...
            <p className="text-lg text-slate-500 font-light">
              {t('blog.subtitle', 'News, insights, and updates')}
            </p>
          </FadeIn>
        </div>
            ...
            {recentArticles.length === 0 ? (
              <div className="bg-slate-50 border border-dashed border-slate-200 rounded-xl p-12 text-center w-full max-w-2xl">
                <p className="text-slate-500 italic text-lg">
                  {t('blog.emptyMessage', 'No articles available yet.')}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {recentArticles.map(article => (
                  <div
                    key={article.id}
                    className="border border-slate-100 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col"
                  >
                    <img
                      src={article.image}
                      alt={t(article.titleKey, article.title)}
                    ...
                    <div className="p-6 flex flex-col items-start h-full flex-grow">
                      ...
                      <Link
                        to="/blog/article"
                        className="text-sm text-[#8A2BE2] font-medium hover:underline mt-auto"
                      >
