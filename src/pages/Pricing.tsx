        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <FadeIn direction="up">
            <h1 className="text-4xl lg:text-5xl font-bold text-slate-800 mb-4 tracking-tight">
              {t('pricingPage.hero.title', 'Pricing')}
            </h1>
            <h2 className="text-2xl font-light text-slate-500">
              {t('pricingPage.hero.subtitle', 'Choose from our products')}
            </h2>
          </FadeIn>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-6xl flex flex-col lg:flex-row gap-8 items-start relative">
          ...
          <div className="w-full lg:w-2/3 flex flex-col gap-6 pb-16">
            ...
            <div className="flex flex-col gap-4">
              <button 
                ...
                className={`p-6 rounded-xl border-2 transition-all flex items-center justify-between text-left ${product === 'rp' ? 'border-[#8A2BE2] bg-white shadow-md' : 'border-slate-200 bg-white/60 hover:bg-white hover:border-[#8A2BE2]/40'}`}
              >
              ...
              <button 
                ...
                className={`p-6 rounded-xl border-2 transition-all flex items-center justify-between text-left ${product === 'to' ? 'border-[#8A2BE2] bg-white shadow-md' : 'border-slate-200 bg-white/60 hover:bg-white hover:border-[#8A2BE2]/40'}`}
              >
                <div className="flex items-center gap-6">
                  <div className="w-12 h-12 flex items-center justify-center bg-[#f5f3ff] rounded-full shrink-0">
              ...
              <button 
                ...
                className={`p-6 rounded-xl border-2 transition-all flex items-center justify-between text-left ${product === 'rpto' ? 'border-[#8A2BE2] bg-white shadow-md' : 'border-slate-200 bg-white/60 hover:bg-white hover:border-[#8A2BE2]/40'}`}
              >
                <div className="flex items-center gap-6">
            ...
            <div className="mt-8 bg-white p-6 rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="flex w-full overflow-x-auto pb-4">
                 <div className="flex flex-col w-[35%] shrink-0 pr-4 gap-4 text-sm font-light text-slate-600">
                   ...
                   { val: 50, label: '50+', disabled: false }
                    ...
                    return (
                      <button 
                        ...
                        onClick={() => setUsersTier(tier.val as any)}
                      ...
                      >
                         <div className="h-16 flex flex-col items-center justify-end pb-2 gap-2">
                            ...
                            <span className={isActive ? 'text-[#8A2BE2]' : 'text-slate-600'}>{tier.label}</span>
                         </div>
                         
                         <div className={`h-10 flex items-center justify-center w-full ${isActive ? 'text-[#489E46]' : 'text-slate-400'}`}>
            ...
            <div className="mt-4 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
               ...
               {showIntegrations && (
                 <div className="p-5 border-t border-slate-100 flex flex-col gap-3">
                    ...
                    <a href="#" className="text-[#8A2BE2] hover:underline text-sm font-medium mt-2 flex items-center gap-1">
                      {t('pricingPage.configurator.integrations.link')} <ChevronRight className="w-4 h-4" />
                    </a>
                 </div>
               )}
            </div>
            ...
            <div className="mt-8">
               ...
               <div className="flex flex-col sm:flex-row gap-6">
                  <button 
                    ...
                    className={`flex-1 p-6 rounded-xl border-2 text-left transition-all ${billing === 3 ? 'border-[#8A2BE2] bg-white shadow-md' : 'border-slate-200 bg-white/60 hover:bg-white'}`}
                  >
                     <div className="flex items-center justify-between mb-2">
                        ...
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${billing === 3 ? 'border-[#8A2BE2]' : 'border-slate-300'}`}>
                  ...
                  <button 
                    ...
                    className={`flex-1 p-6 rounded-xl border-2 text-left transition-all relative overflow-hidden ${billing === 12 ? 'border-[#8A2BE2] bg-white shadow-md' : 'border-slate-200 bg-white/60 hover:bg-white'}`}
                  >
                     <div className="flex items-center justify-between mb-2">
                        <span className={`text-xl font-bold ${billing === 12 ? 'text-[#8A2BE2]' : 'text-slate-700'}`}>{t('pricingPage.configurator.billing.m12')}</span>
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${billing === 12 ? 'border-[#8A2BE2]' : 'border-slate-300'}`}>
                           {billing === 12 && <div className="w-2.5 h-2.5 rounded-full bg-[#8A2BE2]"></div>}
                        </div>
                     </div>
                     {savings && (
                       <p className="text-[#489E46] font-bold text-sm">
          ...
          <div className="w-full lg:w-1/3 sticky top-28 bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-100 flex flex-col z-20">
             ...
             <div className="p-8 pb-10 flex flex-col items-center text-center">
                <h3 className="text-2xl font-light text-slate-600 mb-6 w-full text-left">{t('pricingPage.configurator.stickyBox.title')}</h3>
                
                {displayPrice === 'request' ? (
                  <div className="flex flex-col items-center justify-center py-4 text-center">
                     <h4 className="text-3xl font-bold text-[#8A2BE2] mb-4">50+ licenses</h4>
             ...
             <div className="bg-[#8A2BE2] p-8 text-white flex flex-col items-start text-left">
          ...
          <FadeIn direction="up">
             ...
             <div className="grid md:grid-cols-2 gap-x-12 gap-y-6 text-left max-w-5xl mx-auto">
                {[
                  ...
                  "How secure is S-Locator?"
                ].map((q, i) => (
                  <a key={i} href="/faq" className="text-[17px] font-medium text-[#8A2BE2] hover:text-[#489E46] transition-colors py-2 border-b border-slate-100 leading-snug block">
                    {q}
                  </a>
                ))}
             </div>
