...
const CostItem: React.FC<{ icon: React.ReactNode; amount: string; label: string }> = ({
  icon,
  amount,
  label,
}) => (
    ...
    <div className="max-w-4xl mx-auto mt-12">
      <div className="bg-white rounded-md shadow-sm border border-slate-100 p-8 md:p-12 mb-8 min-h-[250px] flex items-center relative overflow-hidden">
        {testimonials.map((testim, idx) => (
          ...
          >
            ...
            <div
              className={`w-full md:w-2/3 flex flex-col border-l border-slate-200 ${isRTL ? 'text-right pr-8' : 'text-left pl-8'}`}
            >
              <img
                src={testim.logo}
                alt="Company Logo"
                className="h-6 object-contain self-start mb-6 opacity-80 grayscale"
                onError={e => (e.currentTarget.style.display = 'none')}
              />
      ...
      <div className="flex items-center justify-center gap-6">
        <button
          ...
          className="text-[#489E46] hover:text-[#8A2BE2] transition-colors focus:outline-none"
        >
        ...
        <div className="flex gap-2">
          {testimonials.map((_, idx) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all focus:outline-none ${idx === currentSlide ? 'bg-[#8A2BE2]' : 'bg-transparent border border-[#8A2BE2]'}`}
            />
          ))}
        </div>
        <button
...
export const TerritoryCostSavings: React.FC = () => {
  ...
  return (
    <div
      className={`min-h-screen bg-white font-sans text-slate-900 selection:bg-[#f5f3ff] selection:text-[#8A2BE2] ${isRTL ? 'text-right' : 'text-left'}`}
    >
      {/* Hero Section */}
            ...
            <div className="max-w-2xl mx-auto flex flex-col gap-6 text-slate-700 font-light text-[17px] leading-relaxed text-center bg-white/70 backdrop-blur-md p-8 rounded-3xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
              ...
              <div className="py-5 border-y border-purple-100 my-2">
                <p className="text-xl text-slate-500 mb-2">
                  {t('costSavings.hero.scenarioText')}{' '}
                  <span className="font-bold text-[#489E46]">{t('costSavings.hero.reps')}</span>
                </p>
                <p className="text-xl text-slate-500 mb-3">{t('costSavings.hero.achieveText')}</p>
                <p
                  className="text-5xl font-extrabold text-[#8A2BE2] tracking-tight drop-shadow-sm"
                  dir="ltr"
                >
                  €88,600
                </p>
          ...
          <FadeIn direction="up">
            ...
            <div className="flex flex-wrap justify-center gap-4 lg:gap-8 max-w-5xl mx-auto">
              <CostItem
                ...
                amount="€51,480"
                label={t('costSavings.breakdown.salary')}
              />
              <CostItem
                icon={<FileText size={32} strokeWidth={1.5} />}
                amount="€14,400"
                label={t('costSavings.breakdown.taxes')}
              />
              <CostItem
                icon={<Car size={32} strokeWidth={1.5} />}
                amount="€11,520"
                label={t('costSavings.breakdown.car')}
              />
              <CostItem
                icon={<Users size={32} strokeWidth={1.5} />}
                amount="€6,600"
                label={t('costSavings.breakdown.team')}
              />
              <CostItem
                icon={<Droplet size={32} strokeWidth={1.5} />}
                amount="€4,320"
                label={t('costSavings.breakdown.fuel')}
              />
              <CostItem
                icon={<Building size={32} strokeWidth={1.5} />}
                amount="€3,840"
                label={t('costSavings.breakdown.hotel')}
              />
              <CostItem
                icon={<GraduationCap size={32} strokeWidth={1.5} />}
                amount="€3,840"
                label={t('costSavings.breakdown.training')}
              />
              <CostItem
                icon={<Laptop size={32} strokeWidth={1.5} />}
                amount="€2,280"
                label={t('costSavings.breakdown.it')}
              />
              <CostItem
                icon={<Search size={32} strokeWidth={1.5} />}
                amount="€2,040"
                label={t('costSavings.breakdown.hr')}
              />
            </div>
          </FadeIn>
        </div>
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          <FadeIn direction="up">
            <div className="hidden md:flex w-full mb-6 font-bold text-slate-500 text-lg">
              ...
              <div className="w-[40%] text-center text-[#8A2BE2]">
                {t('costSavings.comparison.slocTitle')}
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row w-full mb-4">
              <div className="w-full md:w-[40%] bg-slate-200/50 p-6 rounded-l-lg border border-slate-200 flex flex-col items-center justify-center text-center mb-2 md:mb-0">
                <span className="text-3xl font-light text-slate-700 mb-1" dir="ltr">
                  €3,000,000
                </span>
                <span className="text-sm font-light text-slate-500">
                  {t('costSavings.comparison.convTotal')}
                </span>
              </div>
              <div className="w-full md:w-[20%] bg-[#8A2BE2] text-white p-6 border-y md:border-y-0 md:border-x border-white flex flex-col items-center justify-center text-center shadow-lg z-10 relative md:-mx-2">
                <span className="text-2xl font-bold mb-1" dir="ltr">
                  €100,000
                </span>
                <span className="text-sm font-light text-purple-200">
                  {t('costSavings.comparison.savingsBox')}
                </span>
              </div>
              <div className="w-full md:w-[40%] bg-white p-6 rounded-r-lg border border-[#8A2BE2]/20 flex flex-col items-center justify-center text-center shadow-sm">
                <span className="text-3xl font-light text-[#8A2BE2] mb-1" dir="ltr">
                  €2,900,000
                </span>
                <span className="text-sm font-light text-[#489E46]">
                  {t('costSavings.comparison.slocTotal')}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row w-full mb-16">
              <div className="w-full md:w-[40%] bg-slate-100 p-6 rounded-l-lg border border-slate-200 flex flex-col items-center justify-center text-center mb-2 md:mb-0">
                <span className="text-xl font-light text-slate-700 mb-1" dir="ltr">
                  €3,000
                </span>
                <span className="text-xs font-light text-slate-500">
                  {t('costSavings.comparison.convSoft')}
                </span>
              </div>
              <div className="w-full md:w-[20%] bg-slate-400 text-white p-6 border-y md:border-y-0 md:border-x border-white flex flex-col items-center justify-center text-center z-10 relative md:-mx-2">
                <span className="text-lg font-bold mb-1" dir="ltr">
                  €11,400
                </span>
                <span className="text-xs font-light text-slate-100">
                  {t('costSavings.comparison.addCostBox')}
                </span>
              </div>
              <div className="w-full md:w-[40%] bg-white p-6 rounded-r-lg border border-slate-200 flex flex-col items-center justify-center text-center shadow-sm">
                ...
                <span className="text-xs font-light text-slate-500">
                  {t('costSavings.comparison.slocSoft')}
                </span>
              </div>
            </div>
            ...
            <div className="text-center">
              <p className="text-2xl text-slate-600 font-light mb-4">
                {t('costSavings.comparison.totalSavingsTitle')}
              </p>
              <p
                className="text-[64px] font-bold text-[#8A2BE2] leading-none drop-shadow-sm"
                dir="ltr"
              >
                €88,600
              </p>
      ...
      <section className="py-24 bg-[#f5f3ff]/60 border-b border-purple-100">
        <div className="container mx-auto px-4 max-w-6xl text-center">
          <FadeIn direction="up">
            <h2 className="text-3xl lg:text-4xl font-bold text-[#8A2BE2] mb-16">
              {t('costSavings.advantages.title')}
            </h2>
            ...
            <div className="grid md:grid-cols-3 gap-12 mb-16">
              <div className="flex flex-col items-center">
                ...
                <h4 className="text-xl font-bold text-slate-800 mb-4">
                  {t('costSavings.advantages.effTitle')}
                </h4>
                <p className="text-slate-600 font-light leading-relaxed">
                  {t('costSavings.advantages.effDesc')}
                </p>
              </div>
              <div className="flex flex-col items-center">
                <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-100 mb-6">
                ...
                <h4 className="text-xl font-bold text-slate-800 mb-4">
                  {t('costSavings.advantages.costTitle')}
                </h4>
                <p className="text-slate-600 font-light leading-relaxed">
                  {t('costSavings.advantages.costDesc')}
                </p>
              </div>
              <div className="flex flex-col items-center">
                <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-100 mb-6">
                ...
                <h4 className="text-xl font-bold text-slate-800 mb-4">
                  {t('costSavings.advantages.compactTitle')}
                </h4>
                <p className="text-slate-600 font-light leading-relaxed">
                  {t('costSavings.advantages.compactDesc')}
                </p>
              </div>
            </div>
      ...
      <section className="py-24 bg-white border-b border-slate-200 text-center">
        <div className="container mx-auto px-4">
          <FadeIn direction="up">
            <h2 className="text-3xl lg:text-[36px] font-bold text-[#8A2BE2] mb-12">
              {t('territoryAdvantages.testimonials.title', 'Success stories of our customers')}
            </h2>
        ...
        <div className="container mx-auto px-4 relative z-10 max-w-4xl">
          <FadeIn direction="up">
            ...
            <p className="text-purple-100 text-lg font-light mb-10 max-w-2xl mx-auto">
              {t(
                'territoryAdvantages.demoCta.desc',
                'Our team will be happy to show you the features of S-Locator Territory Optimization.',
              )}
            </p>

            <div className="flex flex-wrap justify-center gap-6 mb-10">
              <span className="text-[#489E46] bg-white px-4 py-1.5 rounded-full text-sm uppercase tracking-wider font-bold">
                {t('territoryAdvantages.demoCta.f1', 'Free')}
              </span>
              <span className="text-[#489E46] bg-white px-4 py-1.5 rounded-full text-sm uppercase tracking-wider font-bold">
                {t('territoryAdvantages.demoCta.f2', '30-60 min')}
              </span>
              <span className="text-[#489E46] bg-white px-4 py-1.5 rounded-full text-sm uppercase tracking-wider font-bold">
                {t('territoryAdvantages.demoCta.f3', 'Online')}
              </span>
            </div>
      ...
      <section className="py-24 bg-white border-b border-slate-200">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <FadeIn direction="up">
            <h2 className="text-3xl lg:text-4xl font-bold text-[#8A2BE2] mb-6">
              {t(
                'territoryAdvantages.pricing.title',
                'S-Locator Territory Optimization license models',
              )}
            </h2>
            <p className="text-lg text-[#489E46] font-medium max-w-2xl mx-auto mb-16">
              {t(
                'territoryAdvantages.pricing.desc',
                'If you have a team of at least 2 field reps...',
              )}
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
              <div className="bg-[#f5f3ff] p-10 rounded-xl shadow-sm border border-purple-100 flex flex-col items-center">
                ...
                <div className="text-2xl font-bold text-slate-800 mb-6">
                  {t('territoryAdvantages.pricing.m3', 'Months')}
                </div>
                <hr className="w-16 border-t-2 border-[#489E46] mb-6" />
                <p className="text-[#8A2BE2] font-light mb-8 flex-grow">
                  {t('territoryAdvantages.pricing.m3Desc', 'Ideal for short-term projects.')}
                </p>
              ...
              <div className="bg-[#8A2BE2] p-10 rounded-xl shadow-md border border-[#8A2BE2] flex flex-col items-center relative">
                <div
                  className={`absolute top-0 ${isRTL ? 'left-0 rounded-br-lg rounded-tl-xl' : 'right-0 rounded-bl-lg rounded-tr-xl'} bg-[#489E46] text-white text-xs font-bold px-3 py-1`}
                >
                  {t('territoryAdvantages.pricing.popular', 'POPULAR')}
                </div>
                <div className="text-6xl font-light text-white mb-2">12</div>
                <div className="text-2xl font-bold text-white mb-6">
                  {t('territoryAdvantages.pricing.m12', 'Months')}
                </div>
                <hr className="w-16 border-t-2 border-[#489E46] mb-6" />
                <p className="text-purple-100 font-light mb-8 flex-grow">
                  {t('territoryAdvantages.pricing.m12Desc', 'Ideal for continuous optimization.')}
                </p>
      ...
      <section className="bg-[#f8fafc] py-24 text-center">
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <h3 className="text-xl text-[#489E46] font-medium">
          ...
          <div className="flex flex-col md:flex-row gap-8 max-w-5xl mx-auto mt-16 mb-20">
            <div className="flex-1 bg-[#f5f3ff]/50 p-10 rounded-xl shadow-sm border border-purple-100 text-center flex flex-col items-center">
              ...
              <div
                className={`text-left flex flex-col gap-4 text-[#8A2BE2] font-medium text-[15px] w-full max-w-xs mx-auto ${isRTL ? 'items-start text-right' : 'items-start text-left'}`}
              >
            ...
            <div className="flex-1 bg-[#f5f3ff]/50 p-10 rounded-xl shadow-sm border border-purple-100 text-center flex flex-col items-center">
              ...
              <div
                className={`text-left flex flex-col gap-4 text-[#8A2BE2] font-medium text-[15px] w-full max-w-xs mx-auto mt-2 ${isRTL ? 'items-start text-right' : 'items-start text-left'}`}
              >
                <div className="flex items-center gap-3" dir="ltr">
