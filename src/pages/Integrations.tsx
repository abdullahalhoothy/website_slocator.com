...
import { FileSpreadsheet, Cloud, LayoutDashboard, Database, AppWindow, Code, Network, CheckCircle2, Phone, Mail, MonitorSmartphone, ChevronRight } from 'lucide-react';
                  ...
                  return (
                     ...
                     >
                        <div className="flex items-center gap-4">
                           <div className={`p-2 rounded-lg ${isActive ? 'bg-[#f5f3ff]' : 'bg-slate-100 text-slate-400'}`}>
                              {tab.icon}
                           </div>
                           {t(`integrationsPage.tabs.${tab.key}.title`)}
                        </div>
                        <ChevronRight className={`w-5 h-5 ${isRTL ? 'rotate-180' : ''} ${isActive ? 'text-[#8A2BE2]' : 'text-slate-300'}`} />
                     </button>
                  );
               })}
            </div>
               ...
               <FadeIn key={activeTab} direction="up" duration={0.4}>
                  <div className="bg-white rounded-2xl p-8 lg:p-12 shadow-sm border border-slate-200 h-full flex flex-col">
                     ...
                     <ul className="flex flex-col gap-5 flex-grow mb-10">
                        {Array.isArray(activeTabData.items) && activeTabData.items.map((item, idx) => (
                           <li key={idx} className="flex items-start gap-4">
                              <CheckCircle2 className="w-6 h-6 text-[#489E46] shrink-0 mt-0.5" strokeWidth={2} />
         ...
         <div className="container mx-auto px-4 max-w-6xl">
            <FadeIn direction="up">
               ...
               <div className="grid md:grid-cols-3 gap-8">
                  {/* Blog Card 1 */}
                  <a href="#" className="bg-slate-50 rounded-xl overflow-hidden border border-slate-200 hover:shadow-lg transition-all group flex flex-col h-full">
