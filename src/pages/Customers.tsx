...
export const Customers: React.FC = () => {
  ...
  const stories = [
    {
      ...
      quote: t(
        'customersPage.stories.s1.quote',
        'S-Locator not only helps us with planning, but above all with bringing transparency to our field sales operations.',
      ),
      author: 'Philip Wilms',
    ...
    },
    {
      ...
      quote: t(
        'customersPage.stories.s2.quote',
        'With S-Locator, scheduling is no longer part of the job. It simply happens automatically.',
      ),
      author: 'Oscar George',
    ...
    },
    {
      ...
      quote: t(
        'customersPage.stories.s3.quote',
        'With S-Locator, our field reps save 30 to 45 minutes per day in route planning and preparation.',
      ),
      author: 'Jonas Seidl',
    ...
    },
    {
      ...
      quote: t(
        'customersPage.stories.s4.quote',
        'A dynamic company such as Samsung requires a dynamic route planner! Call frequencies are now in firm control thanks to S-Locator.',
      ),
    ...
    },
    {
      ...
      quote: t(
        'customersPage.stories.s5.quote',
        'SONAX sales representatives like working with S-Locator because they can reschedule flexibly on the road and the software is very user-friendly.',
      ),
    ...
    },
    {
      ...
      quote: t(
        'customersPage.stories.s6.quote',
        'With S-Locator we get an extra 20% visits to accounts in any given week. S-Locator is a huge time-saver for us.',
      ),
      author: 'Ian Henry',
    ...
    },
    {
      ...
      quote: t(
        'customersPage.stories.s7.quote',
        "I wouldn't want to be without S-Locator now. The software is worth the money because when you factor in the labor costs of manual processing, it pays for itself.",
      ),
    ...
    },
    {
      ...
      quote: t(
        'customersPage.stories.s8.quote',
        'Thanks to S-Locator, we can quickly turn our field sales requirements into reality and get optimized results at the touch of a button.',
      ),
      author: 'Jonas Gronau',
    ...
    },
    {
      ...
      quote: t(
        'customersPage.stories.s9.quote',
        'For us, S-Locator is the ultimate solution because it simply works and adapts perfectly to our everyday working life.',
      ),
      author: 'Moritz Weiss',
  ...
  const reviews = [
    {
      author: 'Murray Godden',
      text: t(
        'customersPage.reviews.r1',
        'The best system Sales and Marketing could ever purchase for hitting those execution goals, been using it for over 6yrs.',
      ),
    },
    {
      author: 'Clint Mallum',
      text: t(
        'customersPage.reviews.r2',
        'This is an amazing application. It helps us save a lot of time on the road and focus on our customers.',
      ),
    },
    {
      author: 'Beren Brechtel',
      text: t(
        'customersPage.reviews.r3',
        'Pretty great. The product is excellent and helps our team hit their monthly execution targets efficiently.',
      ),
    },
    {
      author: 'Philippe SAUNIER',
      text: t(
        'customersPage.reviews.r4',
        'Just awesome! Time saving, optimization, efficiency, performance... No hassle. Totally recommended.',
      ),
    },
    {
      author: 'Alberto Bertamino',
      text: t(
        'customersPage.reviews.r5',
        'Unique tool. It adapts perfectly to our specific needs and the support team is always available.',
      ),
    },
    {
      author: 'Frank Burek',
      text: t(
        'customersPage.reviews.r6',
        'Relaxed route planning! It takes the stress out of my daily routine.',
      ),
    },
  ]
          ...
          <FadeIn direction="up">
            ...
            <div className="flex flex-wrap justify-center gap-x-6 gap-y-4 mb-16">
              {categories.map((cat, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-2 text-sm text-[#8A2BE2] font-medium bg-[#f5f3ff] px-3 py-1.5 rounded-full"
                >
                  {cat.icon}
                  <span>{cat.label}</span>
                </div>
        ...
        <div className="container mx-auto px-4 max-w-7xl">
                ...
                <div className="relative bg-white rounded-2xl shadow-sm border border-slate-200 p-8 pt-16 hover:shadow-xl transition-all duration-300 h-full flex flex-col group">
                  ...
                  <div className="h-12 flex justify-center items-center mb-6 mt-2">
                    <img
                      ...
                      onError={e => {
                        e.currentTarget.src = 'https://via.placeholder.com/150x40?text=Logo'
                      }}
                    />
                  </div>
          ...
          <div className="flex justify-center mt-16 gap-2" dir="ltr">
            {[1, 2, 3, 4].map(page => (
              <button
                key={page}
                className={`w-10 h-10 rounded-full font-bold flex items-center justify-center transition-colors ${page === 1 ? 'bg-[#8A2BE2] text-white shadow-md' : 'bg-white border border-slate-200 text-slate-600 hover:bg-[#f5f3ff] hover:text-[#8A2BE2]'}`}
              >
                {page}
              </button>
            ))}
          ...
          <FadeIn direction="up">
            <div className="flex gap-6 overflow-x-auto pb-8 snap-x snap-mandatory scrollbar-hide">
              {reviews.map((review, idx) => (
                <div
                  key={idx}
                  className="min-w-[300px] md:min-w-[350px] bg-white p-8 rounded-2xl shadow-sm border border-slate-100 snap-center shrink-0 flex flex-col"
                >
                  <p className="font-bold text-slate-800 mb-2">{review.author}</p>
                  <div
                    className={`flex text-[#f0ad4e] mb-4 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}
                  >
                    {[...Array(5)].map((_, i) => (
      ...
      <section className="bg-[#8A2BE2] py-24 text-center relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 max-w-4xl">
          <FadeIn direction="up">
            ...
            <p className="text-purple-200 text-sm font-light">{t('customersPage.cta.note')}</p>
          </FadeIn>
        </div>
            ...
            <div className="grid md:grid-cols-2 gap-x-16 gap-y-6 text-left max-w-5xl mx-auto">
              {faqQuestions.map((q, i) => (
                <a
                  key={i}
                  href="/faq"
                  className={`text-[17px] font-medium text-[#8A2BE2] hover:text-[#489E46] transition-colors py-3 border-b border-slate-100 leading-snug block ${isRTL ? 'text-right' : 'text-left'}`}
                >
                  {q}
                </a>
              ))}
            ...
            <div className="flex-1 bg-white p-10 rounded-2xl shadow-sm border border-slate-100 text-center flex flex-col items-center">
              ...
              <div className="flex flex-col gap-4 text-slate-700 font-medium text-[15px] items-start w-full max-w-xs mx-auto">
                ...
                <div className="flex items-center gap-3 mt-2 text-slate-600">
                  ...
                  {t('contactPage.support.teamviewer', 'Teamviewer: Remote access')}
                </div>
              </div>
            </div>
            <div className="flex-1 bg-white p-10 rounded-2xl shadow-sm border border-slate-100 text-center flex flex-col items-center">
